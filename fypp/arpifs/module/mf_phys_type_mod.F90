...
!>REAL (KIND=JPRB)                      :: FPFPCG (NPROMA, 0:YDCPG_OPTS%KFLEVG)
  TYPE (FIELD_3D), POINTER :: F_FPFPCG => NULL ()
...
SELF%F_FPFPCG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_OPTS%KFLEVG+1, NLEV0=0, PERSISTENT=PERSISTENT)
...
SELF%F_SIC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, PERSISTENT=PERSISTENT)
...
IF (ASSOCIATED (SELF%F_FPFPCG)) CALL DELETE_TEMPORARY (SELF%F_FPFPCG) 
...
IF (ASSOCIATED (SELF%F_SIC)) CALL DELETE_TEMPORARY (SELF%F_SIC) 
