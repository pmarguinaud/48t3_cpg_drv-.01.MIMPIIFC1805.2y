...
IF (YDCPG_OPTS%LTWOTL) THEN
  ...
  IF( .NOT.LLCT )THEN
    ...
    IF ((YDCPG_OPTS%NSTEP <= 0).OR.YDML_DYN%YRDYNA%LNESC) THEN
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          YDCPG_SL1%DPHI9%P(JROF,JLEV)=PESGM*ZRDPHI(JROF,JLEV)
          YDCPG_SL1%DBBC9%P(JROF,JLEV)=&
    ...
    ELSEIF (YDML_DYN%YRDYNA%LSETTLS) THEN
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ...
          YDCPG_SL1%DPHI9%P(JROF,JLEV)=ZMIXNL*ZSETTLS+(1.0_JPRB-ZMIXNL)*ZNESC
          ...
          YDCPG_SL1%DBBC9%P(JROF,JLEV)=ZMIXNL*ZSETTLS+(1.0_JPRB-ZMIXNL)*ZNESC 
        ENDDO
      ENDDO
    ELSE
      ! * remaining case: lsettls=false, lnesc=false.
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          YDCPG_SL1%DPHI9%P(JROF,JLEV)=PESGM*&
           & (1.5_JPRB*ZRDPHI(JROF,JLEV)-0.5_JPRB*YDVARS%DPHI%T9(JROF,JLEV))
          YDCPG_SL1%DBBC9%P(JROF,JLEV)=PESGM*&
    ...
    YDCPG_SL1%GWS9%P=PGWS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
  ...
  ELSE
    ...
    IF (.NOT.LLCTC) THEN
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          YDCPG_SL1%DPHI9%P(JROF,JLEV)=PESGM*YDVARS%DPHI%T9(JROF,JLEV)
          YDCPG_SL1%DBBC9%P(JROF,JLEV)=&
    ...
    ! * Calculation of PB1(.,MSLB1GWS9):
    IF (.NOT.LLCTC) YDCPG_SL1%GWS9%P=YDVARS%GWS%T9(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
...
IF (.NOT.YDCPG_OPTS%LTWOTL) THEN
  ...
  ! * Calculation of PB1(.,MSLB1GWS9):
  IF (.NOT.LLCT) YDCPG_SL1%GWS9%P=PGWS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
...
IF (.NOT.LLCTC) THEN
  YDCPG_SL1%DPHI9%P(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,0)=0.0_JPRB
  YDCPG_SL1%DPHI9%P(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,NFLEVG+1)=0.0_JPRB
  YDCPG_SL1%DBBC9%P(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,0)=0.0_JPRB
  YDCPG_SL1%DBBC9%P(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,NFLEVG+1)=0.0_JPRB
ENDIF
