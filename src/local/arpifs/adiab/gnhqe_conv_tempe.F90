...
ELSE
  ...
  LLDER2=.FALSE.
ENDIF
LLMOIST_RCP=(PRESENT(KGFLTYP).AND.PRESENT(YDVARS))
...
! * Compute Kap=R/Cp:
IF (LLMOIST_RCP) THEN
  CALL GPRCP_EXPL(YDCST,NPROMA,KST,KEND,NFLEVG,YDVARS=YDVARS,PKAP=ZKAP,KGFLTYP=KGFLTYP)
ELSE
  ZKAP(KST:KEND,1:NFLEVG)=YDCST%RD/YDCST%RCPD
ENDIF

! * Compute exp(Kap*Qcha):
CALL GNHPRE(YDDYNA%NPDVAR,YDGEOMETRY,NPROMA,NFLEVG,KST,KEND,PQCHA,ZPREF,PKAP=ZKAP,PEQCHAF=ZEQCHAF)
