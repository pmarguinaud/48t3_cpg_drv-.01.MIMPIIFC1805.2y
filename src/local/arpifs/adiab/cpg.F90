...
IF (CDPART (1:1) == 'X') THEN
  ...
  IF (YDCPG_OPTS%LSLPHY.AND.((YDMODEL%YRML_DYN%YRDYNA%LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LPC_FULL))) THEN
    ...
    & YDCPG_BNDS%KFDIA, YDCPG_OPTS%LFSTEP, YDCPG_OPTS%ZDTPHY, YDCPG_SL1%UP9%P, YDCPG_SL1%VP9%P, &
    & YDCPG_SL1%TP9%P, YDCPG_SL1%ZVIEW(:, MSLB1GFLP9), PSAVTEND, PGFLSLP)
  ENDIF
...
IF (CDPART (3:3) == 'X') THEN
  ...
  IF (LSLAG) THEN
    CALL CPG_DYN_SLG (YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_MISC, YDCPG_DYN0, YDCPG_DYN9, &
                    & YDVARS, YDCPG_SL1, YDCPG_SL2, YDMODEL, SLHDA(:,YDCPG_BNDS%KBL), SLHDD0(:,YDCPG_BNDS%KBL), &
  ...
  IUPTRA = GET_NUPTRA ()
  CALL CPG_END_SFC (YDMODEL, YDCPG_OPTS, YDMF_PHYS_SURF, IUPTRA)
