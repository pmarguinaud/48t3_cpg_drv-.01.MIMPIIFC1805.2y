...
IF (YDCPG_OPTS%NSTEP > 0 .AND. NCURRENT_ITER==0) THEN
  ...
  IF (YDML_DYN%YRDYNA%LSLINLC2) THEN
    DO JLEV = 1,NFLEVG
      DO JROF = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ...
        YDCPG_SL1%C9_SI%P(JROF,JLEV)=YDCPG_SL1%C9_SI%P(JROF,JLEV)&
  ...
  IF (YDML_DYN%YRDYNA%LSLINL) THEN
    DO JLEV = 1,NFLEVG
      DO JROF = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ZMIXNL = PMIXNL(JROF, JLEV)
        YDCPG_SL1%U9_SI%P(JROF,JLEV)=YDCPG_SL1%U9_SI%P(JROF,JLEV)&
         & + ZMIXNL*(ZBT*PGAGT0L(JROF,JLEV)-YDVARS%UNL_SI%T9(JROF,JLEV))
        YDCPG_SL1%V9_SI%P(JROF,JLEV)=YDCPG_SL1%V9_SI%P(JROF,JLEV)&
         & + ZMIXNL*(ZBT*PGAGT0M(JROF,JLEV)-YDVARS%VNL_SI%T9(JROF,JLEV))
        YDCPG_SL1%T9_SI%P(JROF,JLEV)=YDCPG_SL1%T9_SI%P(JROF,JLEV)&
         & + ZMIXNL*(ZBDT(JROF)*PTOD0(JROF,JLEV)-YDVARS%TNL_SI%T9(JROF,JLEV))
        YDCPG_SL1%VD9_SI%P(JROF,JLEV)=YDCPG_SL1%VD9_SI%P(JROF,JLEV)&
         & + ZMIXNL*(ZBT*PLPD0(JROF,JLEV)-YDVARS%SVDNL_SI%T9(JROF,JLEV))
      ENDDO
      IF (YDCPG_OPTS%LNHEE) THEN
        DO JROF = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZMIXNL = PMIXNL(JROF, JLEV)
          YDCPG_SL1%PD9_SI%P(JROF,JLEV)=YDCPG_SL1%PD9_SI%P(JROF,JLEV)&
