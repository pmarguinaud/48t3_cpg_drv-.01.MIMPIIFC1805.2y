...
INTEGER(KIND=JPIM) :: JGFL,JVAR,JWVL,IGFLPTR,INCR,JCOMPO_3D_FID
...
ASSOCIATE(LADER=>YDDIMF%LADER, &
 ...
 & LTRCMFP=>YGFL%LTRCMFP, NCRM=>YGFL%NCRM, NUMFLDS=>YGFL%NUMFLDS, &
 & YCRM=>YGFL%YCRM, YCRM_NL=>YGFL%YCRM_NL, &
...
!      1.1  Initial settings.
  ...
  ENDIF
  IF (NCRM == 0) THEN
    YGFL%YCRM => YGFL%YCOMP(JPGFL-INCR:JPGFL-INCR)
    INCR=INCR+1
  ENDIF
...
!      1.2  Grid-point GFL.
  ...
  IF(YTKE_NL%LGP) THEN
    ...
    CALL DEFINE_GFL_COMP(YGFL, YDCOMP=YGFL%YTKE ,CDNAME=YFATKE%CLNAME,KGRIB=NGRBSNR,&
  ...
  JCOMPO_3D_FID = FID%COMPO_3D_FIRST
  ...
  DO JGFL=1,NGHG
    IF(YGHG_NL(JGFL)%LGP) THEN
      ...
      CALL DEFINE_GFL_COMP(YGFL, YDCOMP=YGFL%YGHG(JGFL),CDNAME=YGHG_NL(JGFL)%CNAME ,&
       ...
       & LDNEGFIX=YGHG_NL(JGFL)%LNEGFIX,PWENO_ALPHA=YGHG_NL(JGFL)%WENO_ALPHA, KFID=JCOMPO_3D_FID)
  ...
  DO JGFL=1,NCHEM
    IF(YCHEM_NL(JGFL)%LGP) THEN
      ...
      CALL DEFINE_GFL_COMP(YGFL, YDCOMP=YGFL%YCHEM(JGFL),CDNAME=YCHEM_NL(JGFL)%CNAME ,&
       ...
       & KGRIBTC=YCHEM_NL(JGFL)%IGRIBTC,KGRIBSFC=YCHEM_NL(JGFL)%IGRIBSFC,KGRBFLXO=YCHEM_NL(JGFL)%IGRBFLXO, &
       ...
       & LDNEGFIX=YCHEM_NL(JGFL)%LNEGFIX,PWENO_ALPHA=YCHEM_NL(JGFL)%WENO_ALPHA,KFID=JCOMPO_3D_FID)
      JCOMPO_3D_FID=JCOMPO_3D_FID + 1
    ENDIF
  ENDDO
  ...
  IF (JCOMPO_3D_FID-1 > FID%COMPO_3D_LAST) THEN
  ...
  DO JGFL=1,NCRM
    ...
    IF(YCRM_NL(JGFL)%LGP) THEN
      IF(JGFL == 1) THEN
        IGFLPTR=NUMFLDS+1
        YGFL%YCRM=>YGFL%YCOMP(IGFLPTR:IGFLPTR+NCRM-1)
      ENDIF
      CALL DEFINE_GFL_COMP(YGFL, YDCOMP=YGFL%YCRM(JGFL),CDNAME=YCRM_NL(JGFL)%CNAME ,&
       & KGRIB=YCRM_NL(JGFL)%IGRBCODE, LDGP=.TRUE.,  KREQIN=YCRM_NL(JGFL)%NREQIN ,&
       & PREFVALI=YCRM_NL(JGFL)%REFVALI, LDREQOUT=YCRM_NL(JGFL)%LREQOUT ,&
       & LDERS=LLDERS, LD5=.FALSE., LDT1=.FALSE.,LDGPINGP=.TRUE. ,&
       & LDTRAJIO=.FALSE.)
    ENDIF
  ENDDO
...
!      1.3  Spectral GFL.
  ...
  ! CRM fields
  DO JGFL=1,NCRM
    IF(YCRM_NL(JGFL)%LSP) THEN
      IF(JGFL == 1) THEN
        IGFLPTR=NUMFLDS+1
        YGFL%YCRM=>YGFL%YCOMP(IGFLPTR:IGFLPTR+NCRM-1)
      ENDIF
      CALL DEFINE_GFL_COMP(YGFL, YDCOMP=YGFL%YCRM(JGFL),CDNAME=YCRM_NL(JGFL)%CNAME ,&
       & KGRIB=YCRM_NL(JGFL)%IGRBCODE, LDGP=.FALSE.,  KREQIN=0 ,&
       & PREFVALI=YCRM_NL(JGFL)%REFVALI, LDREQOUT=YCRM_NL(JGFL)%LREQOUT ,&
       & LDERS=LADER, LD5=.FALSE., LDT1=.FALSE.)
    ENDIF
  ENDDO
