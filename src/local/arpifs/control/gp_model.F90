...
& YDGEM=>YDGEOMETRY%YRGEM, YDDIMV=>YDGEOMETRY%YRDIMV, YDDYN=>YDMODEL%YRML_DYN%YRDYN,   YDPTRSLB1=>YDMODEL%YRML_DYN%YRPTRSLB1,&
& YDPTRSLB2=>YDMODEL%YRML_DYN%YRPTRSLB2,YDEPHLI=>YDMODEL%YRML_PHY_SLIN%YREPHLI,   YDLDDH=>YDMODEL%YRML_DIAG%YRLDDH,          &
& YDSLPHY=>YDMODEL%YRML_PHY_G%YRSLPHY,YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY,   YDRADF=>YDMODEL%YRML_PHY_RAD%YRRADF,               &
& YDRIP=>YDMODEL%YRML_GCONF%YRRIP,YDPHY2=>YDMODEL%YRML_PHY_MF%YRPHY2,YGFL=>YDMODEL%YRML_GCONF%YGFL,                          &
& YDTRC=>YDMODEL%YRML_PHY_RAD%YRTRC,YDMDDH=>YDMODEL%YRML_DIAG%YRMDDH,   YDECUCONVCA=>YDMODEL%YRML_PHY_EC%YRECUCONVCA,        &
& YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY,   YDGPDDH=>YDMODEL%YRML_DIAG%YRGPDDH,YDCHEM=>YDMODEL%YRML_CHEM%YRCHEM,                 &
& YDDPHY=>YDMODEL%YRML_PHY_G%YRDPHY,YDERAD=>YDMODEL%YRML_PHY_RAD%YRERAD)

ASSOCIATE(NUMFLDS=>YGFL%NUMFLDS,   LCHEM_ANAO3=>YDCHEM%LCHEM_ANAO3, KCHEM_NOXADV=>YDCHEM%KCHEM_NOXADV,                             &
& NGPBLKS=>YDDIM%NGPBLKS, NPROMA=>YDDIM%NPROMA,NFLEVG=>YDDIMV%NFLEVG,   LFINDVSEP=>YDDYN%LFINDVSEP, LGPSTRESS=>YDDYN%LGPSTRESS,    &
& NCURRENT_ITER=>YDDYN%NCURRENT_ITER, NSITER=>YDDYN%NSITER,   NVSEPC=>YDDYN%NVSEPC, NVSEPL=>YDDYN%NVSEPL,                          &
& LCUCONV_CA=>YDECUCONVCA%LCUCONV_CA,   LPHYLIN=>YDEPHLI%LPHYLIN,   LAGPHY=>YDEPHY%LAGPHY, LEGBRAD=>YDEPHY%LEGBRAD,                &
& LEPHYS=>YDEPHY%LEPHYS,   LERADI=>YDEPHY%LERADI, LERAINGG=>YDEPHY%LERAINGG,   LESMOS=>YDEPHY%LESMOS,                              &
& NPRACCL=>YDEPHY%NPRACCL, LSLPHY=>YDEPHY%LSLPHY,  NRADFR=>YDERAD%NRADFR,   NGPTOT=>YDGEM%NGPTOT, NGPTOT_CAP=>YDGEM%NGPTOT_CAP,    &
& GFL=>YDGFL%GFL, GFLPC=>YDGFL%GFLPC, GFLPT=>YDGFL%GFLPT, GFLSLP=>YDGFL%GFLSLP,   GMV=>YDGMV%GMV, GMVS=>YDGMV%GMVS,                &
& YT0=>YDGMV%YT0,   YT1=>YDGMV%YT1, YT9=>YDGMV%YT9,   GFLTNDHD_DDH=>YDGPDDH%GFLTNDHD_DDH, GFLTNDSL_DDH=>YDGPDDH%GFLTNDSL_DDH,      &
& GMVTNDHD_DDH=>YDGPDDH%GMVTNDHD_DDH, GMVTNDSI_DDH=>YDGPDDH%GMVTNDSI_DDH,   GMVTNDSL_DDH=>YDGPDDH%GMVTNDSL_DDH,                    &
& NFLDSLB1=>YDPTRSLB1%NFLDSLB1, RPARSL1=>YDPTRSLB1%RPARSL1,   NFLDSLB2=>YDPTRSLB2%NFLDSLB2,   EMTD=>YDRADF%EMTD,                   &
& EMTU=>YDRADF%EMTU, RMOON=>YDRADF%RMOON, TRSW=>YDRADF%TRSW,   NSTOP=>YDRIP%NSTOP,   SAVTEND=>YDSLPHY%SAVTEND,                     &
& SD_DI=>YDSURF%SD_DI, SD_PF=>YDSURF%SD_PF, SD_SFL=>YDSURF%SD_SFL,   SD_SFO=>YDSURF%SD_SFO, SD_SM=>YDSURF%SD_SM,                   &
& SD_V2=>YDSURF%SD_V2,   SD_V3=>YDSURF%SD_V3, SD_VA=>YDSURF%SD_VA, SD_VC=>YDSURF%SD_VC,   SD_VD=>YDSURF%SD_VD,                     &
& SD_VF=>YDSURF%SD_VF, SD_VH=>YDSURF%SD_VH,   SD_VN=>YDSURF%SD_VN, SD_VP=>YDSURF%SD_VP, SD_VV=>YDSURF%SD_VV,                       &
& SD_WS=>YDSURF%SD_WS, SD_WW=>YDSURF%SD_WW, SD_X2=>YDSURF%SD_X2,   SD_XA=>YDSURF%SD_XA, SP_EP=>YDSURF%SP_EP,                       &
& SP_OM=>YDSURF%SP_OM,   SP_RR=>YDSURF%SP_RR, SP_SB=>YDSURF%SP_SB, SP_SG=>YDSURF%SP_SG,   SP_SL=>YDSURF%SP_SL,                     &
& SP_X2=>YDSURF%SP_X2, SP_CL=>YDSURF%SP_CL,   SD_XP=>YDSURF%SD_XP, SD_XP2=>YDSURF%SD_XP2,LMPHYS=>YDPHY%LMPHYS,                     &
& TSPHY=>YDPHY2%TSPHY,   SD_VK=>YDSURF%SD_VK,   NFIELDS3D_AUTO=>YDMDDH%NFIELDS3D_AUTO,NFIELDS3D_OFFSET=>YDMDDH%NFIELDS3D_OFFSET,   &
& NFIELDS2D_AUTO=>YDMDDH%NFIELDS2D_AUTO,NFIELDS2D_OFFSET=>YDMDDH%NFIELDS2D_OFFSET,   LFLEXDIA=>YDLDDH%LFLEXDIA,                    &
& LDDH_OMP=>YDLDDH%LDDH_OMP, YDSL=>YDMODEL%YRML_DYN%YRSL,   LGRADHPHY=>YDMODEL%YRML_PHY_MF%YRARPHY%LGRADHPHY                       &
& ) 
!     ------------------------------------------------------------------
!*       1.    INITIALISATION
!     ------------------------------------------------------------------
